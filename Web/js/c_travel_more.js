				    ...
				    success: function (o) {
				        if (o.Return == 0) {			            
				            ...
				            for (var i = 0; i < o.List.length; i++) {
				                ...
				                tempHtml = tempHtml.replace("{cName}", oi.name);
				                $("#listPanel").append(tempHtml);
				                obj.append("<li><a href=\"#panel_" + oi.id + "\"><span>" + oi.name + "</span></a></li>");
				            }
				        }
				        else {
				            tempHtml = template.replace(/{id}/g, _Travel.ID);
				            tempHtml = tempHtml.replace("{cName}", "全部列表");
				            $("#listPanel").append(tempHtml);
				        }
				        _Travel.getList();
				    }
        ...
        $.ajax(
				{
				    ...
				    dataType: "json",
				    cache: false,
				    data: { fn: 1,cId:_Travel.ID, t: new Date() },
				    success: function (o) {
				        $.hideLoading();
				        if (o.Return == 0) {
				            var isChd = 0;
