var _List = {
    ...
    Init: function () {
        ...
        if (_List.oType != null && _List.oType == 1) {
            $(".weui-navbar").find("a").removeClass("weui-bar__item--on");
            $(".weui-navbar").find("a").eq(1).addClass("weui-bar__item--on");
            $(".weui-tab__bd-item").removeClass("weui-tab__bd-item--active");
            $(".weui-tab__bd-item").eq(1).addClass("weui-tab__bd-item--active");
        }
                        ...
                        if (isH && o.length > 0) {
                            for (var i = 0; i < o.length; i++) {
                                ...
                                if (order.Order_state == 2 && order.ZuoHao_state == 0) {
                                    ...
                                    tempHtml = tempHtml.replace("{hide}", "hidden");
                                }
                                if (order.Money_return == 0 && order.Order_state == 0) {
                                    tempHtml = tempHtml.replace("{payhide}", "");
                                }
                                else {
                                    tempHtml = tempHtml.replace("{payhide}", "hidden");
                                }
                            ...
                            obj.find(".weui-btn_warn").click(function () {
                                _List.isOpenInfo = 1;
                            ...
                            obj.find(".weui-btn_primary").click(function () {
                                var no = $(this).attr("data-no");
                            ...
                            obj.find(".weui-btn_default").click(function () {
                                var orderno = $(this).attr("data-no");
                                _List.no = orderno;
        ...
        $.ajax(
                {
                    ...
                    dataType: "json",
                    cache: false,
                    data: { fn: 101, uid: $.cookie("seascape_def_USER_ID"), t: new Date() },
                    success: function (o) {
                        var tempHtml = "";
                        obj_train.html("");
                        var fName = "";
