var _Trip = {
    ...
    initSelect: function () {
        if ($.cookie("Trip_select") != null) {
            ...
            $("#topCityBar").find(".am-u-sm-8").find("span").eq(0).text(_F.sCity);
            $("#topCityBar").find(".am-u-sm-8").find("span").eq(2).text(_F.tCity);
            _Trip.Select();
        }
        else {
    ...
    initLayot: function () {
        $("#topBar").find(".am-u-sm-3").click(function () {
            ...
            success: function (o) {
                ...
                if (o.INFO.AIR != null) {
                    ...
                    for (var i = 0; i < searchFlihtNum; i++) {
                        ...
                        //航班信息
                        sPort = a.depart_city.toString().replace("机场", "机场").replace(_F.sCity, "");
                        if (sPort.length == 0) {
                            sPort = a.depart_city.toString().replace("机场", "机场");
                        }
                        tPort = a.arrive_city.toString().replace("机场", "机场").replace(_F.tCity, "");
                        if (tPort.length == 0) {
                            tPort = a.arrive_city.toString().replace("机场", "机场");
                        }
                        ...
                        for (var j = 0; j < f.class.seat.length; j++) {
                            ...
                            if (ps == "A" || !isNaN(ps)) {
                                if (c.price.length > 0 && parseInt(c.price) > 0) {
                                    ...
                                    cm = cm.replace("{bgrule}", bgrule);
                                    tprule = "";
                                    for (var key in c.refund) {
                                        tprule = c.refund[key];
                                    }
                                    cm = cm.replace("{tprule}", tprule);
                                    ...
                                    if (isLow) {
                                        ...
                                        tm = tm.replace("{lowPrice}", c.price.replace(".00", ""));
                    ...
                    //最低价格1(周四)<em>￥298元</em>
                    if (_Trip.s == 2) {
                        $("#topBar").find(".am-u-sm-6").find("span").html(_F.tDate.substr(5, 5) + " " + _C.getWeek(_F.tDate));
                        $("#topBar").find(".am-u-sm-6").find("em").html("￥" + lowPrice + "元");
                    }
                    else {
                        $("#topBar").find(".am-u-sm-6").find("span").html(_F.sDate.substr(5, 5) + " " + _C.getWeek(_F.sDate));
                        $("#topBar").find(".am-u-sm-6").find("em").html("￥" + lowPrice + "元");
                    }
                    if (flihtNum == 0) {
                        $("#topBar").find(".am-u-sm-6").find("span").html(_F.sDate.substr(5, 5) + " " + _C.getWeek(_F.sDate));
                        $("#topBar").find(".am-u-sm-6").find("em").html("￥--元");
                    ...
                    _F.isAfter = 1;
                    $("#topBar").find(".am-u-sm-3").eq(0).css("color","#fff");
                    if (s == 0) {
                        $("#topBar").find(".am-u-sm-3").eq(0).css("color", "#bbb");
                        _F.isAfter = 0;
                    }
