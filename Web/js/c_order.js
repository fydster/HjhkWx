var _Order = {
                ...
                if (o.Return == 0) {
                    if (o.List != null) {
                        ...
                        obj.find("li").find(".rightPrice").click(function () {
                            var type = $(this).attr("data-type");
                            var rule = "<p>使用细则：</p><p>1、	该优惠券仅限在“山西出行”公众号提交订单的用户使用；</p><p>2、	该优惠券限2017.5.31日前使用有效，每张优惠券限使用一次，且每张订单限使用一张优惠券，此优惠不可与其他优惠活动同时使用，不可叠加使用，不找零，不可兑换现金；</p><p>3、	用户在享受优惠立减后，若发生退票、改签等情况，则以实际支付金额为准，优惠立减金额不予退还；</p><p>4、	如发生恶意盗刷优惠券行为，“山西出行”公众号有权收回该行为涉及的优惠券，并取消该账号已获取的优惠。</p>";
                            if (type == 1) {
                                rule = "<p>使用细则：</p><p>1、	该优惠券仅限在“山西出行”公众号提交订单的用户使用；</p><p>2、	该优惠券仅限购买贵宾服务产品时使用；</p><p>3、	该优惠券限2017.5.31日前使用有效，每张优惠券限使用一次，且每张订单限使用一张优惠券，此优惠不可与其他优惠活动同时使用，不可叠加使用，不找零，不可兑换现金；</p><p>4、	用户在享受优惠立减后，若发生退票、改签等情况，则以实际支付金额为准，优惠立减金额不予退还；</p><p>5、	如发生恶意盗刷优惠券行为，“山西出行”公众号有权收回该行为涉及的优惠券，并取消该账号已获取的优惠。</p>";
                            }
    ...
    showFliht: function () {
        var isTy = 0;
        if (_F.sType == 1) {
            var Obj = $(".flihtInfo").find(".info");
            Obj.find(".hb span").eq(0).html(_F.flihtName + _F.flihtNo);
            Obj.find(".hb span").eq(1).html(_F.jixing);
            Obj.find(".hb span").eq(2).html(_F.cw);

            var aObj = Obj.find(".am-u-sm-5");
            aObj.eq(0).find(".date").text(_F.sDate);
            aObj.eq(0).find(".date").find("em").text(_C.getWeek(_F.sDate));
            aObj.eq(0).find(".time").text(_F.sTime);
            aObj.eq(0).find(".air").text(_F.sPort + _F.sT);

            aObj.eq(1).find(".date").text(_F.sDate);
            aObj.eq(1).find(".date").find("em").text(_C.getWeek(_F.sDate));
            aObj.eq(1).find(".time").text(_F.tTime);
            aObj.eq(1).find(".air").text(_F.tPort + _F.tT);
            ...
            var pObj = Obj.find(".am-u-sm-4");
            pObj.eq(0).find("em").text("￥" + _F.price);
            pObj.eq(1).find("em").text("￥" + _F.jj);
            pObj.eq(2).find("em").text("￥" + _F.tax);
    ...
    getPrice: function () {
        ...
        $("#topBar").find(".am-u-sm-4").eq(0).text("￥" + total);
