				    ...
				    success: function (o) {
				        if (o.Return == 0) {
				            ...
				            for (var i = 0; i < o.List.length; i++) {
				                ...
				                var oi = o.List[i];
				                if (oi.pId == 0) {
				                    tempHtml = template_0.replace("{1}", n.toString());
				                    tempHtml = tempHtml.replace("{5}", "<img src=\"" + oi.logoUrl + "\" width=\"60\"\"/>");
				                    tempHtml = tempHtml.replace("{6}", btnTemplate.replace(/{id}/g, oi.id).replace(/{pName}/g, oi.name).replace(/{fId}/g, oi.pId).replace(/{sType}/g, oi.sType).replace(/{isShow}/g, oi.isShow).replace("{logoUrl}", oi.logoUrl));
				                    ...
				                    for (var j = 0; j < o.List.length; j++) {
				                        ...
				                        if (oj.pId == oi.id) {
				                            ...
				                            tempHtml = tempHtml.replace("{6}", btnTemplate.replace(/{id}/g, oj.id).replace(/{pName}/g, oj.name).replace(/{fId}/g, oj.pId).replace(/{sType}/g, oi.sType).replace(/{isShow}/g, oi.isShow));
